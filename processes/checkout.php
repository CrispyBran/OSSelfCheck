...
if (!empty($_SESSION['patron_barcode'])){
	...
	//Check if Renewal has been requested by user
	if (!empty($_POST['renew'])){
		...
		$cko = $mysip->msgPatronInformation('charged');
		$info = $mysip->parsePatronInfoResponse($mysip->get_message($cko));
		$found = '0';
		foreach ($info['variable']['AU'] as $check){
			if ($check==$item_barcode){
				$found = '1';
			}
		}
		
		if ($found=='1') {
			if (in_array($item_barcode,$_SESSION['cko_barcodes'])) {
				$popupmsg='You already scanned this item.';
	...
	echo '
	<tr>
	<!-- display list number next to title -->
